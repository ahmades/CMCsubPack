...
C
   ...
   15 IFLAG = 5
      ...
      SUBROUTINE BRACKET(F,X1,X2,EXPFACT)
C     GIVEN A FUNCTION F AND AN INITIAL GUESSED RANGE FROM X1 TO X2, THE SUBROUTINE BRACKET EXPANDS 
C     THE RANGE [X1,X2] GEOMETRICALLY USING THE EXPANSION FACTOR EXPFACT UNTIL A ROOT IS 
C     BRACKETED. THE BOUNDS OF THE ROOT BRACKETING RANGE ARE RETURNED IN X1 AND X2.
      DOUBLE PRECISION X1,X2,F,F1,F2,EXPFACT
      EXTERNAL F
      INTEGER TEST
C     CHECK IF BOUNDS OF INITIAL GUESSED RANGE ARE DISTINCT
      IF(X1.EQ.X2) THEN
         WRITE(*,*)'ERROR IN SUBROUTINE BRACKET'
         WRITE(*,*)'BOUNDS OF GUESSED INITIAL RANGE ARE EQUAL'
      ...
      TEST = 0
      DO WHILE(TEST.EQ.0)
         IF((F1*F2).LT.0.0D0) THEN ! F CROSSES [X1,X2]. EXPANSION NOT NEEDED ANYMORE.
            TEST = 1
         ELSE ! F DOES NOT CROSS [X1,X2]. EXPAND [X1,X2].
